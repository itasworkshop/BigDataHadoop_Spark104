...
resultdf = sparkSession.sql("select n.stockticker,c.sector,sum(n.volume) from temp_nyse n, temp_comp c where n.stockticker = c.symbol group by n.stockticker,c.sector order by sum(n.volume) desc")
